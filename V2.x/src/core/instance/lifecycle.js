...
/** 
 * 模板将会被编译成渲染函数，接着创建一个渲染函数的观察者，从而对渲染函数求值，在求值的过程中会触发数据对象 name 属性的 get 拦截器函数，
...
export function mountComponent (
  ...
  let updateComponent
  /* istanbul ignore if */
  // 该段代码的作用只有一个，即定义并初始化updateComponent函数
  // 这个函数将用作创建Watcher实例时传递给Watcher构造函数的第二个参数
  // if...else语句块的作用只是做性能统计，实际功能不变
  ...
  // updateComponent函数的执行会间接触发渲染函数(vm.$options.render)的执行
  // 渲染函数的执行则会触发数据属性的get拦截器函数，从而将依赖(观察者)收集
  // 当数据变化时将重新执行updateComponent函数，这就完成了重新渲染
